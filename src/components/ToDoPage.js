...
class ToDoPage extends Component {
  ...
  handleCheck(key) {
    ...
    const updatedClass =
      value[index].cName === 'todoTextFalse'
    ...
    const listItems = this.state.value.map(v => (
      <Segment className="TodoItem" key={v.key}>
        <Grid columns={'equal'}>
          <Grid.Column width={1}>
            <Checkbox
              checked={v.cName === 'todoTextTrue'}
              onClick={e => this.handleCheck(v.key)}
            />
          </Grid.Column>
          ...
          <Grid.Column width={5} textAlign="centered">
            <Grid.Row>
              <div className="todo-segment">
                <Segment.Group>{listItems}</Segment.Group>
              </div>
              <ToDoForm
                show={this.state.showModal}
                onClose={this.handleModal}
                onUpdate={this.handleInputFromToDo}
